...
def admin_keyboard(admin_id=None):
    notify_label = (
        "🔔 New User Notif: ON"
        if (admin_id is None or db.get_admin_notify(admin_id))
        else "🔕 New User Notif: OFF"
    )
...
async def ask_for_screenshot(bot, chat_id):
    ...
    await bot.send_message(
        chat_id=chat_id,
        text=(
            "📸 *𝙎𝙀𝙉𝘿 𝙎𝘾𝙍𝙀𝙀𝙉𝙎𝙃𝙊𝙏 𝙊𝙁 𝙔𝙊𝙐𝙍 𝙋𝘼𝙔𝙈𝙀𝙉𝙏 𝙁𝙊𝙍 𝙂𝙀𝙏 𝙋𝙍𝙀𝙈𝙄𝙐𝙈*\n\n"
...
async def send_payment_qr(bot, chat_id, cat):
    ...
    phonpe = "https://phon.pe/ru_" + _up.quote(f"{upi}&am={price}", safe="")
    paytm  = f"https://paytm.com/biz/pay?pa={upi}&am={price}"
    gpay   = "https://gpay.app.goo.gl/pay"
...
async def post_proof_to_channel(bot, photo_id, user):
    ...
    caption = (
        "🎉 *A user just bought Indian Pack* 🇮🇳\n\n"
        "✅ Payment verified & confirmed!\n\n"
        "💎 *Want Premium Access too?*\n"
        "👇 Click the button below to buy now!"
    )
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("💎 Get Premium — Buy Now", url=bot_link)
    ]])
    ...
    try:
        ...
        log.info(f"✅ Proof posted to channel {proof_channel}")
    except Exception as e:
        log.error(f"❌ Could not post proof to channel {proof_channel}: {e}")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🔹 *Set Proof Link:* Set the link shown when user taps Payment Proofs.\n"
        "🔹 *Set Proof Channel:* Set channel ID/username where screenshots auto-post.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if adm:
        ...
        if state == "wait_proof_link" and text:
            ...
            db.set_state(user.id, "none")
            await message.reply_text(
                f"✅ Proof link updated to: {text.strip()}",
                reply_markup=admin_keyboard(user.id)
            )
            return
        ...
        if state == "wait_proof_channel" and text:
            ...
            db.set_state(user.id, "none")
            await message.reply_text(
                f"✅ Proof channel set to: {text.strip()}\n\n"
                f"Make sure the bot is an *admin* in that channel so it can post!",
                parse_mode="Markdown",
                reply_markup=admin_keyboard(user.id)
            )
            return
        ...
        if state.startswith("wait_price_val_") and text:
            ...
            if text.isdigit():
                ...
                await message.reply_text(
                    f"✅ Price for {display} updated to ₹{text}!",
                    reply_markup=admin_keyboard(user.id)
                )
            else:
                ...
                try:
                    if photo:
                        await context.bot.send_photo(
                            chat_id=uid, photo=photo[-1].file_id,
                            caption=message.caption or ""
                        )
                    elif video:
                        await context.bot.send_video(
                            chat_id=uid, video=video.file_id,
                            caption=message.caption or ""
                        )
                    elif text:
    ...
    if text == "Payment proofs 📋":
        ...
        if proof_link:
            await message.reply_text(
                "*Payment Proofs ✅*\n\nClick below to view verified payment proofs:",
                parse_mode="Markdown",
            ...
            await message.reply_text(
                "*Payment proofs channel not set yet.*\nContact support.",
                parse_mode="Markdown"
            )
        return
    ...
    if adm:
        ...
        if text == "Change UPI":
            ...
            await message.reply_text(
                "Send the channel/group link for Payment Proofs:\n"
        ...
        # NEW: Set Proof Channel button
            ...
            await message.reply_text(
                "Send the *channel username or ID* where payment screenshots will be auto-posted.\n\n"
                "Examples:\n"
                "• `@MyProofChannel`\n"
                "• `-1001234567890`\n\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "get_premium":
        ...
        demos = db.get_demos()
        if not demos:
            await context.bot.send_message(chat_id=chat_id, text="No demo videos available right now.")
        else:
            for (did, fid) in demos:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton("👉 Get Premium", callback_data="get_premium")
